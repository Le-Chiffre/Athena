...
// Converts a 4-bit value to a hexadecimal character.
template<typename T>
inline T charFromHex(Byte hex) {
    assert(hex <= 0xF);
    if(hex >= 0xA) {
        return 'A' + (hex - 0xA);
    } else {
...
template<typename T>
inline T* stringFromHashHelper(U32 hash, T* out) {
    out[0] = charFromHex<T>((hash & 0xF0000000) >> 28);
    out[1] = charFromHex<T>((hash & 0x0F000000) >> 24);
    out[2] = charFromHex<T>((hash & 0x00F00000) >> 20);
    out[3] = charFromHex<T>((hash & 0x000F0000) >> 16);
    out[4] = charFromHex<T>((hash & 0x0000F000) >> 12);
    out[5] = charFromHex<T>((hash & 0x00000F00) >> 8);
    out[6] = charFromHex<T>((hash & 0x000000F0) >> 4);
    out[7] = charFromHex<T>((hash & 0x0000000F));
    out[8] = 0;
...
char* stringFromHash(U32 hash, Char* out) {
    return stringFromHashHelper(hash, out);
}
...
void Hasher::addString(const WChar* string) {
    U32 hash = this->hash;
    while(*string) {
        hash ^= SBox_Table[*((Byte*)string)];
        hash *= 3;
        hash ^= SBox_Table[*(((Byte*)string) + 1)];
        hash *= 3;
        string++;
    }
...
void Hasher::addString(const WChar32* string) {
    U32 hash = this->hash;
    while(*string) {
        hash ^= SBox_Table[*((Byte*)string)];
        hash *= 3;
        hash ^= SBox_Table[*(((Byte*)string) + 1)];
        hash *= 3;
        hash ^= SBox_Table[*(((Byte*)string) + 2)];
        hash *= 3;
        hash ^= SBox_Table[*(((Byte*)string) + 3)];
        hash *= 3;
        string++;
    }
